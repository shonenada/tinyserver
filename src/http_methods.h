#ifndef _HTTP_METHODS
#define _HTTP_METHODS

void HTTP_OPTIONS(unsigned char* , unsigned char* , int );
void HTTP_GET(unsigned char*, unsigned char*, int);
void HTTP_HEAD(unsigned char*, unsigned char*, int);
void HTTP_POST(unsigned char*, unsigned char*, int);
void HTTP_PUT(unsigned char*, unsigned char*, int);
void HTTP_DELETE(unsigned char*, unsigned char*, int);
void HTTP_TRACE(unsigned char*, unsigned char*, int);
void HTTP_CONNECT(unsigned char*, unsigned char*, int);
...
void HTTP_OPTIONS(unsigned char* request, unsigned char* ptr, int sockfd)
{
    // TODO
}
...
void HTTP_GET(unsigned char* request, unsigned char* ptr, int sockfd)
{

    unsigned char resource[500];
    // if the requesting resource end with "/", it mean it is requiring a
    // directory.
    if (ptr[strlen(ptr) - 1] == '/') {
        // TODO, try to add a setting in settings.h to make the server 
        // search for more than one index file, such as: index.html, index.htm
        strcat(ptr, DEFAULT_INDEX);
    }

    // let resource to be starting with the value of WEBROOT.
    strcpy(resource, WEBROOT);

    // connect resource and ptr which refer to a requesting file.
    strcat(resource, ptr);
    
    if (ptr == request + 4) {
        sendResource (resource, sockfd, 0);
    }
...
void HTTP_HEAD(unsigned char* request, unsigned char* ptr, int sockfd)
{
    // TODO
}
...
// Other) response can be used to direct the user agent to retrieve a cacheable 
// resource.
//  
...
void HTTP_POST(unsigned char* request, unsigned char* ptr, int sockfd)
{
    // TODO
}
...
// the PUT.
void HTTP_PUT(unsigned char* request, unsigned char* ptr, int sockfd)
{
    // TODO
}
...
// more currently cached entities, those entries SHOULD be treated as stale. 
// Responses to this method are not cacheable.
void HTTP_DELETE(unsigned char* request, unsigned char* ptr, int sockfd)
{
    // TODO
}
...
// value of the Via header field (section 14.45) is of particular interest, 
...
void HTTP_TRACE(unsigned char* request, unsigned char* ptr, int sockfd)
{
...
void HTTP_CONNECT(unsigned char* request, unsigned char* ptr, int sockfd)
{
